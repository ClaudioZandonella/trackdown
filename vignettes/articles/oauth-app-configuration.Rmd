...
```
1. Next, we need to specify the setting of the consent screen. From the left menu select *"APIs and services" > "OAuth consent screen"*
```{r, out.width="90%"}
knitr::include_graphics('images/04-consent-screen.png')
```
  Select *"External"* as User Type
```{r, out.width="90%"}
knitr::include_graphics('images/05-consent-external.png')
```
  Specify the App information *"App name"* and *"User support email"* (use your email)
```{r, out.width="90%"}
knitr::include_graphics('images/06-consent-info.png')
```
  Specify also the developer contact information (use your email) and press *"SAVE AND CONTINUE"*
```{r, out.width="90%"}
knitr::include_graphics('images/07-consent-contact.png')
```
1. Next, we need to select the required scopes. Click *"ADD OR REMOVE SCOPES"*
```{r, out.width="90%"}
knitr::include_graphics('images/08-scopes.png')
```
  Select the required scopes *".../auth/userinfo.email"* and *"openid"*
```{r, out.width="90%"}
knitr::include_graphics('images/09-scopes-list.png')
```
...
```{r, out.width="90%"}
knitr::include_graphics('images/10-test-user.png')
```
...
```{r, out.width="90%"}
knitr::include_graphics('images/11-publish-app.png')
```
1. Now we need to activate the required APIs. Form the *"Enable APIs and services"* panel, click *"ENABLE APIS AND SERVICES"*
```{r, out.width="90%"}
knitr::include_graphics('images/12-api.png')
```
   Using the API Library search service we need to select two APIs
```{r, out.width="50%"}
knitr::include_graphics('images/13-api-search.png')
```
  Select *"Google Drive API"* from the list and click *"ENABLE"*
```{r, out.width="90%"}
knitr::include_graphics('images/14-api-drive.png')
```
  Select *"Google Docs API"* from the list and click *"ENABLE"*
```{r, out.width="90%"}
knitr::include_graphics('images/15-api-docs.png')
```
1. Next, we need to obtain the credentials to use our newly created app. Form the *"Credentials"* panel, click *"CREATE CREDENTIALS"* and select *"OAuth client ID"*
```{r, out.width="90%"}
knitr::include_graphics('images/16-credentials.png')
```
  As *"Application type"*, select *"Desktop app"* and specify the client app name. Next click *"CREATE"*
```{r, out.width="50%"}
knitr::include_graphics('images/17-credentials-type.png')
``` 
...
```{r, out.width="50%"}
knitr::include_graphics('images/18-credentials-popout.png')
```
...
```{r, out.width="70%"}
knitr::include_graphics('images/19-credentials-client.png')
```
