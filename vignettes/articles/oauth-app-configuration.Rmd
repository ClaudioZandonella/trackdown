...
```
  Click *"Update"* at the end of the list to save changes. As we selected sensitive scopes, a pop-out will appear. Do not worry, these scopes are required to managed files in Google Drive. Click *"CONTINUE"*
    ...
    > After selcting all the required scopes, we should obtain the following summary list
    >   ```{r, out.width="60%"}
...
```
1. Now, we need to obtain the credentials to use our newly created app. Form the *"Credentials"* panel, click *"CREATE CREDENTIALS"* and select *"OAuth client ID"*
```{r, out.width="90%"}
